...
**Install AIP, fixed version** (Claude Code, project-local, pinned to `v0.3a0`):

```bash
git clone --depth 1 --branch v0.3a0 https://github.com/zach-blumenfeld/aip.git ./.claude/skills/aip
```

Replace `v0.3a0` with whichever release you want — see [tags](https://github.com/zach-blumenfeld/aip/tags) for the list.
...
The AIP protocol version (currently `v0.3a0`) is referenced in **multiple places** that must stay in sync. When bumping (e.g., `v0.3a0` → `v0.3`):
