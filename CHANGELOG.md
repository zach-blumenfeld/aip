...
- **Body** may now carry a prose preamble before the single fenced YAML block, so a default agent without the aip server knows how to follow the graph. Nothing may follow the block.
